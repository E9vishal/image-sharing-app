class Photo < ActiveRecord::Base
  attr_accessor :image, :title, :album_id, :remote_image_url
  ...
  def max_images_count
    errors[:base] << "Images count can not exceed 25 for an album." if album.image_count >= 25
  end
  
  after_create do |photo|
    photo.album.increment!(:image_count)
  end
  after_destroy do |photo|
    photo.album.decrement!(:image_count)
  end
